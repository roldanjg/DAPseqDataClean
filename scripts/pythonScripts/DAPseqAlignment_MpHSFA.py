...
with open(ids_file, 'r') as samplesOntology:
    idsDf = pd.read_csv(samplesOntology, names=['rawindex', 'tf', 'time','cond','rep'])

with cd(working_folder):
    for index, attri in idsDf.iterrows():
        gzs = []
        targetFolder = os.path.join(str(attri.tf),str(attri.time),str(attri.cond),str(attri.rep))
            ...
            if checkMD5isCorrect(originalfolder):
                ...
                if checkFastaQLenght(originalfolder):
                    ...
                    if qualityCheckTrimGalore(targetFolder):
                        samfileexperiment = \
                        f'{str(attri.tf)}{str(attri.time)}{str(attri.cond)}{str(attri.rep)}.sam'
