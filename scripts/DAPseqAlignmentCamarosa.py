...
ids_file = '../data/commonData/ids_camarosa.csv'
working_folder = '../data/fragaria_camarosa_data/'
raw_folder = '../raw_fragaria_camarosa'
working_folder_name = 'fragaria_camarosa_data'
bowtie2mode = '--sensitive'

with open(ids_file, 'r') as samplesOntology:
    idsDf = pd.read_csv(samplesOntology, names=['id', 'tf', 'species'])

with cd(working_folder):
    for index, id in idsDf.iterrows():
        gzs = []
        targetFolder = os.path.join(id.tf, str(id.species))
            ...
            if checkMD5isCorrect(originalfolder):
                ...
                if checkFastaQLenght(originalfolder):
                    ...
                    if qualityCheckTrimGalore(targetFolder):
                        samfileexperiment = '{}{}.sam'.format(id.tf, str(id.species))
                        ...
                        if id.tf == 'Input':
                            ...
                            inputControlpath = os.path.join(
                                '/home/joaquin/projects/methylation/data', working_folder_name, 'Input/{}'.format(id.species)
                            )
